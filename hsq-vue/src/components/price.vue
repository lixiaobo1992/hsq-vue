...
const priceType = device.alipay && api.getCommonParams('channel') === 'huabei' ? '花呗价：' : ''
...
export default {
  ...
  computed: {
    priceArr() {
      return Number(this.$props.price * 0.01)
        .toFixed(2)
        .split('.')
    },
  },
  ...
  render(h) {
    const { priceArr, size, bold, rmb = '¥', point, type = priceType } = this
    ...
    return (
      <span class="now-price">
        ...
        <dfn class="item item-price">
          ...
          <span class={priceClass} style={styles}>
            {priceArr[0]}
          </span>
