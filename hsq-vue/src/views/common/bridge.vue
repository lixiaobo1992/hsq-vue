  ...
  <div class="page-debug">
    ...
    <div class="list" @click="goNext($event)">
      ...
      <div class="content">
        ...
        <button class="btn" data-type="call" data-value="removeNavigationBarMenu">
      ...
      <div class="content">
        ...
        <p>选择事件</p>
        <label class="btn"><input v-model="event" type="radio" value="shake">测试demo</label>
        <label class="btn"><input v-model="event" type="radio" value="back">back</label>
        <!-- <label class="btn"><input type="radio" v-model="event" value="refresh">refresh</label> -->
        <label
          class="btn"
        ><input v-model="event" type="radio" value="pullRefresh">pullRefresh</label>
        <!-- <label class="btn"><input type="radio" v-model="event" value="cityChange">cityChange</label> -->
        <label
          class="btn"
        ><input v-model="event" type="radio" value="locationChanged">locationChanged</label>
    ...
    success(res) {
      if (res.data.type === 'share') {
        bridge.showShare({
          ...
          imgUrl: 'http://img02.tooopen.com/images/20160509/tooopen_sy_161967094653.jpg',
          url: '当前url',
          onlySelectChannel: [
            'Weibo',
            'Weixin',
  ...
  if (!window.msfJsBridge || !window.msfJsBridge.callNative) {
    window.msfJsBridge = {
      callNative(arrMessage) {
        const messages = JSON.parse(arrMessage)
    ...
    goNext(e) {
      ...
      switch (type) {
        ...
        case 'event': {
          ...
          if (value === 'trigger') {
            window.msfJsBridge && window.msfJsBridge.event(`invoke_${this.event}`)
            return
          }
          if (value === 'on') {
