...
function runReadyFn() {
  let len = fnListCaches.length
  console.log('runReadyFn', len)
  for (let i = 0; i < len; i++) {
    fnListCaches[i](window.wx)
  }
  fnListCaches.splice(0, len)
}
...
const wechat = {
  ...
  /* 分享朋友/QQ || 朋友圈/QQ空间 标题 可以分开设置  分开设置是 title为数组 */
  setShare(opts = {}) {
    ...
    const shareData = {
      ...
      weixin_timeline: {
        title: opts.desc || defaultShareInfo.desc,
      },
    ...
    wechat.ready(wx => {
      ...
      // 分享朋友/QQ || 朋友圈/QQ空间 标题需要分开设置

      // 即将被废弃
      wx.onMenuShareAppMessage(dealShareData(shareData)) // 分享给朋友
      wx.onMenuShareTimeline(dealShareData(shareData, 'weixin_timeline')) // 分享到朋友圈
      wx.onMenuShareQQ(dealShareData(shareData, 'qq')) // 分享到QQ

      // !! ios 新版本微信需要调用新的api
      // 自定义“分享给朋友”及“分享到QQ”按钮的分享内容（1.4.0）
      wx.updateAppMessageShareData(dealShareData(shareData))
      // 自定义“分享到朋友圈”及“分享到QQ空间”按钮的分享内容（1.4.0）
      wx.updateTimelineShareData(dealShareData(shareData, 'weixin_timeline'))

      wx.onMenuShareWeibo(dealShareData(shareData)) // 分享到微博
    })
  },
}

function dealShareData(shareData, channel = '') {
  if (shareData[channel]) {
    const shareChannelData = clone(shareData[channel])
    return { ...shareData, ...shareChannelData }
  } else {
    return shareData
  }
}
