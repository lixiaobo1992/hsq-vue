...
import getUrlType from './url-type'
import urlRules from './url-rule'
...
export function hashUrl(url) {
  ...
  const query = Object.assign({}, parse(tempQuery), parse(search.replace('?', '')))
...
export function getTargetUrl(url = '', targetType = 'target') {
  // 获取url，直接拼好参数
  const urlType = getUrlType(url)
  const targetRules = urlRules[camelCase(`${urlType}-to-${targetType}-rules`)] || {}
  // if (!targetRules) {
